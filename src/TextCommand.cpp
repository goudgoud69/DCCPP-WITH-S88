...
void TextCommand::process(){
  char c;
	...
	if (client) {
		...
		INTERFACE.println(F ("HTTP/1.1 200 OK") );
		INTERFACE.println(F ("Content-Type: text/html") );
		INTERFACE.println(F ("Access-Control-Allow-Origin: *") );
		INTERFACE.println(F ("Connection: close") );
		INTERFACE.println("");
...
#ifdef DCCPP_DEBUG_MODE
      Serial.println(""); 
///     Serial.print(F("parse: ")); Serial.println(commandString); // PC
#endif
...
#endif
#ifdef USE_S88
  INTERFACE.print(EEStore::data.nS88);
  INTERFACE.print(">");
#endif
#if !defined(USE_ETHERNET)
	INTERFACE.println("");
